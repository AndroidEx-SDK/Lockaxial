  ...
  render() {
    return (
      <View style={MainStyle.subScreen}>
        <ScrollView>
          <Image source={require('../image/bannerCenter.jpg')} style={{width:windowSize.width,height:(windowSize.width*400/800)}}>
              ...
              <View style={{alignItems:'center'}}>
                {
                  (this.state.userDetail.headimgurl&&this.state.userDetail.headimgurl.length>0)?(
                    <Image style={{marginTop:36,borderRadius:35,width:70,height:70}} source={{uri:this.state.userDetail.headimgurl}}/>
                  ):(
                    <Image style={{marginTop:36,borderRadius:35,width:70,height:70}} source={require('../image/default.png')}/>
                  )
                }
                <Text style={{marginTop:10,color:'white',fontSize:15,backgroundColor: 'rgba(0,0,0,0)'}}>{this.state.userDetail.realname?this.state.userDetail.realname:trans(LABEL_UNKNOW_USER)}</Text>
                <Text style={{marginTop:5,color:'white',fontSize:15,backgroundColor: 'rgba(0,0,0,0)'}}>{this.state.userDetail.mobile?this.state.userDetail.mobile:trans(LABEL_UNKNOW_MOBILE)}</Text>
              </View>
            </TouchableWithoutFeedback>
          </Image>
          <List containerStyle={MainStyle.list}>
            <ListItem title={trans(LABEL_COUPON_INFO)} leftIcon={{name:'youhuiquan',type:'iconfont'}} onPress={()=>this.openCouponScreen()}/>
